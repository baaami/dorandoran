...
const (
	...
	RoomID          = "367c27f2-9a6a-4897-a889-4b2e90617f3b" // 테스트용 RoomID
)
...
func receiveChats(t *testing.T, conn *websocket.Conn, userID string, wg *sync.WaitGroup) {
	...
	for {
		...
		} else if webSocketMsg.Kind == "chat_latest" {
			var chatLatestEvent ChatLatestEvent
			err = json.Unmarshal(webSocketMsg.Payload, &chatLatestEvent)
			if err != nil {
				log.Printf("[ERROR] Failed to unmarshal chat message for User %s: %v", userID, err)
				continue
			}

			log.Printf("[INFO] User %s received event: chat_latest, room: %s", userID, chatLatestEvent.RoomID)
		}
	}
}
